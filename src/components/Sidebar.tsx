...
export default function Sidebar({ onClose, onSearchClick }: SidebarProps) {
  ...
  const [activeWorkspace, setActiveWorkspace] = useState('Apex HQ');

  const workspaces = ['Apex HQ', 'Personal Portfolio', 'DeepMind Labs'];
    ...
    <aside className="flex h-full w-full flex-col border-r border-border bg-card transition-colors duration-200">
      ...
      <div className="relative flex h-16 items-center justify-between px-6 border-b border-border">
        <div className="flex flex-col w-full relative z-30">
          ...
          >
            <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-tr from-indigo-500 via-violet-500 to-indigo-600 shadow-md shadow-indigo-500/20 text-white font-bold text-xs">
            ...
            <div className="flex flex-col flex-1 min-w-0">
              ...
              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Enterprise Workspace</span>
            </div>
          </button>
          ...
          {workspaceDropdownOpen && (
            <>
              ...
              <div className="absolute top-12 left-0 w-52 rounded-xl border border-border bg-card p-1.5 shadow-xl z-20 animate-in fade-in slide-in-from-top-2 duration-150">
                {workspaces.map((ws) => (
                  <button
                    key={ws}
                    onClick={() => {
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-xs font-semibold text-left transition-all ${
                      activeWorkspace === ws ? 'bg-secondary text-foreground' : 'text-muted-foreground hover:bg-secondary/50 hover:text-foreground'
                    }`}
                  >
                    <span>{ws}</span>
                    {activeWorkspace === ws && <Check className="h-3.5 w-3.5 text-indigo-500" />}
                  </button>
                ))}
              </div>
            </>
